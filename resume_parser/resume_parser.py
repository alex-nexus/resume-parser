...
class ResumeParser:
  ...
  def parse(self, resume_paths):
    return [self.parse_one(resume_path) for resume_path in resume_paths]
    # pool = mp.Pool(mp.cpu_count())
    # jobs = [pool.apply_async(self.parse_one, args=(resume_path,)) for resume_path in resume_paths]
    # return [job.get() for job in jobs]

  def parse_one(self, resume_path):
    text = convert_resume_to_text(resume_path)
    doc = self.__nlp(text)
    ...
    return {
        ...
        'mobile': eextract_mobile_number(text),
